  ...
  return declare([BaseWidget],
  {
    ...
    startup: function()
    {
      ...
      function distance(mapz, inputPoints, gl)
      {
        ...
        geometryService.distance(distParams, function(distance)
        {
          ...
          else
          {
            ...
            var extent = esri.graphicsExtent(gl.graphics); 
            ...
            if (totalDistance < 500) {
              var nodeIncorrect = domConstruct.toDom("<img src=widgets/Demo/images/xmark.png align=left><p>Close, you must have minored in Geography. You were off by: " + displayMctires + "km</p>");
              domConstruct.place(nodeIncorrect, "status");
            }
            else if (totalDistance >= 500 && totalDistance < 1000) {
              var nodeIncorrect2 = domConstruct.toDom("<img src=widgets/Demo/images/xmark.png align=left><p>Wow, not even close. You were off by: " + displayMctires + "km</p>");
              domConstruct.place(nodeIncorrect2, "status");
            }
            else {
              var nodeIncorrect3 = domConstruct.toDom("<img src=widgets/Demo/images/xmark.png align=left><p>The goal is to get your answer as close as possible, not the farthest away. You were off by: " + displayMctires + "km</p>");
            ...
            else
            {
              ...
              var exec = dojo.byId('status');
              document.getElementById('distanceDollars').innerHTML = '0';
              dojo.query("#execute").style("visibility", "hidden");
              if (totalQuestions == 1) {
                  dojo.byId('status').innerHTML =totalQuestions + " question?!?! Really?!? Someone is on their way to escort you out of Canada";
              }
              else {
                  dojo.byId('status').innerHTML ="<b>" + "GAME OVER! " + "</b>" + "Congratulations on making it through " + totalQuestions + " questions";
              }
            }
      ...
      function nextQuestion(content)
          ...
          gl.resume();
          gl.clear();
          var initialExtent = new Extent(-17928998.86258547,7204652.627359468,-3840125.809065528,11382394.84531295,new esri.SpatialReference({wkid:102100}));
